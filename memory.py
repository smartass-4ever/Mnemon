...
from .types import (
    ...
    SemanticFact, SignalType, EROS_VERSION
)
...
class SimpleEmbedder:
    """
    Lightweight hash-projection embedder for activation signatures.
    64-dim for fast resonance scoring.
    ...
    def embed_full(self, text: str) -> List[float]:
        """384-dim full semantic embedding for EME templates."""
        tokens = text.lower().split()
        vec = np.zeros(384)
        for token in tokens:
