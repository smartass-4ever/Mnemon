...
Usage:
    ...
    eros = EROS(tenant_id="my_company", agent_id="agent_01")
    await eros.start()
...
from .core.types import (
    ExperienceSignal, MemoryLayer, RiskLevel, SignalType, EROS_VERSION
)
from .core.persistence import EROSDatabase, InvertedIndex
from .core.memory import CognitiveMemorySystem, SimpleEmbedder
from .core.eme import ExecutionMemoryEngine, TemplateAdapter, CostBudget, EMEResult
from .core.bus import ExperienceBus
from .security.manager import SecurityManager, TenantSecurityConfig
...
class EROS:
    def __init__(
        ...
        if memory_enabled:
            self._memory = CognitiveMemorySystem(
                ...
                embedder=self._embedder, llm_client=llm_client,
        ...
        if eme_enabled:
            self._eme = ExecutionMemoryEngine(
                ...
                llm_client=llm_client, adapter=adapter,
    ...
    async def start(self):
        ...
        if self._eme:
            ...
            if self._prewarm_fragments:
                try:
                    from .fragments.library import load_fragments
        ...
        self._started = True
        logger.info(f"EROS {EROS_VERSION} started — tenant={self.tenant_id}")
    ...
    @classmethod
    def from_config(cls, config_path: str = "./eros.config.json") -> "EROS":
        """Load EROS from a config file created by `eros init`."""
        import json
    ...
    def get_stats(self) -> Dict[str, Any]:
        stats: Dict[str, Any] = {"version": EROS_VERSION, "tenant_id": self.tenant_id}
        if self._memory:
...
__all__ = [
    "EROS", "MemoryLayer", "SignalType", "RiskLevel",
    "ExperienceSignal", "CostBudget", "TemplateAdapter",
    "TenantSecurityConfig", "EROS_VERSION"
]
