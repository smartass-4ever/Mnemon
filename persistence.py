...
from .types import (
    ...
    LLMCallLog, MemoryLayer, RiskLevel, EROS_VERSION
)
...
class EROSDatabase:
    ...
    def __init__(self, db_path: str = "eros.db"):
        ...
        async with self._lock:
            self._conn.execute("""
                INSERT INTO audit_log (
                    tenant_id, task_id, timestamp, action, eros_version,
                    template_id, memory_ids, risk_level, human_approved,
                    outcome, component
                ) VALUES (?,?,?,?,?,?,?,?,?,?,?)
            """, (
                entry.tenant_id, entry.task_id, entry.timestamp,
                entry.action, entry.eros_version, entry.template_id,
