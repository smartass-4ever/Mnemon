...
Run: python -m mnemon.eval.harness
or:  mnemon eval --suite standard
...
class EvalReport:
    ...
    def summary(self) -> str:
        ...
        lines = [
            f"Mnemon Eval Report — {self.suite}",
            "=" * 50,
...
class EvalHarness:
    """
    Run eval scenarios against a live Mnemon instance.
    Produces a scorecard — all failing metrics block release.
    """

    def __init__(self, mnemon_instance=None):
        self.mnemon = mnemon_instance
    ...
    async def _eval_retrieval(self, scenario: EvalScenario) -> Tuple[float, str]:
        ...
        if not self.mnemon or not self.mnemon._memory:
            return 1.0, "No memory system — skipped"

        # Pre-load memories from scenario setup
        if scenario.setup.get("memories"):
            from mnemon.core.types import ExperienceSignal, MemoryLayer, SignalType
            import hashlib
            for i, mem_data in enumerate(scenario.setup["memories"]):
                ...
                signal = ExperienceSignal(
                    ...
                    tenant_id=self.mnemon.tenant_id,
                ...
                await self.mnemon._memory.write(signal)

        # Query
        result = await self.mnemon.recall(scenario.query, session_id="eval_session")
    ...
    async def _eval_eme(self, scenario: EvalScenario) -> Tuple[float, str]:
        """Run an EME scenario."""
        if not self.mnemon or not self.mnemon._eme:
    ...
    async def _eval_bus(self, scenario: EvalScenario) -> Tuple[float, str]:
        """Run a bus scenario."""
        if not self.mnemon or not self.mnemon._bus:
        ...
        if scenario.name == "bus_pad_detects_loop":
            ...
            sev = await self.mnemon.report_health(scenario.query, goal=goal)
            if sev is not None:
            ...
            ok = await self.mnemon._bus.belief_registry.set("eval_key", "eval_val", expected_version=0)
            val = await self.mnemon._bus.belief_registry.get("eval_key")
            if val == "eval_val":
            ...
            for i, (task_id, outcome, latency) in enumerate(scenario.setup.get("outcomes", [])):
                await self.mnemon._bus.record_outcome(
                    task_id=task_id, task_type=scenario.query,
                    outcome=outcome, latency_ms=latency
                )
            stats = self.mnemon._bus.tier1.get_stats()
...
async def run_eval(db_path: str = "/tmp/eros_eval.db", suite: str = "standard"):
    """Run eval against a fresh Mnemon instance."""
    import sys
    sys.path.insert(0, ".")
